import { Component, OnInit } from '@angular/core';
import { InformesApiService } from '../informes-api.service';
import { pluck} from 'rxjs/operators';
import { fc03model } from '../informe-ts/fc03/modelo_fc03';
import * as pdfMake from 'pdfmake/build/pdfmake';
import { Prod } from '../informe-ts/descripcion';
import { Prod22 } from '../informe-ts/descripcion22';
import { itemsFc03 } from '../informe-ts/fc03/items_fc03';
import { fc04model } from '../informe-ts/fc04/modelo_fc04';
import { fc10model } from '../informe-ts/fc10/modelo_fc10';
import { itemsFc04 } from '../informe-ts/fc04/items_fc04';
import { Prod2 } from '../informe-ts/descripcion2';

@Component({
  selector: 'app-informe-search',
  templateUrl: './informe-search.component.html'
})
export class InformeSearchComponent implements OnInit {
  public placeholder:String="elige una opción";
  public seleccion!:String;
  public itemSeleccion=["fc03","fc04","fc10"]
  public datofc03!:fc03model[];
  public copydatofc03!:fc03model[];

  public datofc04!:fc04model[];
  public copydatofc04!:fc04model[];

  public datofc10!:fc10model[];
  public copydatofc10!:fc10model[];

  public bien2:Prod[]=[];
  public ImagePath: string= "";
  public searchdatofc03:string="";
  public searchdatofc04:string="";
  public searchdatofc10:string="";

  public itemsRecibido: itemsFc03[] = [];
  public nuevoArray:itemsFc03[]=[];

  public itemsRecibido2: Prod22[] = [];
  public nuevoArray2: Prod22[]=[];

  public itemsRecibido3: Prod2[] = [];
  public nuevoArray3: Prod2[]=[];

  public nuevoInforme={
    cont_informe:0
  }

  public modelofc03:fc03model={
    analitico_1: '',
    analitico_2: '',
    area: '',
    cantidad: 0,
    cuenta: 0,
    dependencia: '',
    descripcion: '',
    entidad: '',
    fecha: '',
    fecha_incorp: '',
    fecha_informe: '',
    items: '',
    iva: 0,
    numero: '',
    numero_informe: '',
    origen: '',
    origen_movi: '',
    reparticion: '',
    rotulado: '',
    signo: '',
    sub_cuenta: 0,
    sub_total: 0,
    tipo: '',
    totales: 0,
    unidad_jerarquica: '',
    valor_total: 0,
    valor_unitario: 0,
    vida_util: '',
    reparticion_cod: '',
    dependencia_cod: '',
    cont_informe: 0
  };
  constructor(private apiService:InformesApiService) { }

  ngOnInit(): void {

    this.ImagePath ='data:image/jpeg;base64,';''

      this.getAllfc03();
      this.getAllfc04();
      this.getAllfc10();


  }
  cambios(){

    this.copydatofc03=[];
    this.copydatofc03 = this.datofc03.filter(x => x.cont_informe == Number(this.searchdatofc03));
  }

  cambios2(){

    this.copydatofc04=[];
    this.copydatofc04 = this.datofc04.filter(x => x.cont_informe == Number(this.searchdatofc04));
  }

  cambios3(){

    this.copydatofc10=[];
    this.copydatofc10 = this.datofc10.filter(x => x.cont_informe == Number(this.searchdatofc10));
  }


  getAllfc03():void{
    this.apiService.getAllfc03().pipe(
      pluck('data')

    )
    .subscribe(data => {
      this.datofc03=data;
      this.copydatofc03=this.datofc03;
    });
  }

  getAllfc04():void{
    this.apiService.getAllfc04().pipe(
      pluck('data')

    )
    .subscribe(data => {
      this.datofc04=data;
      this.copydatofc04=this.datofc04;
    });
  }

  getAllfc10():void{
    this.apiService.getAllfc10().pipe(
      pluck('data')

    )
    .subscribe(data => {
      this.datofc10=data;
      this.copydatofc10=this.datofc10;
    });
  }



generarInforme2(data:string,allData:fc03model){
    this.itemsRecibido=[]
    this.nuevoArray=[]
    this.itemsRecibido=JSON.parse(data);
    this.nuevoArray = this.itemsRecibido;

    console.log("allData:"+allData);

    const exs: { columns: ({ text: number,fontSize:5 } | { text: string,fontSize:5})[][]; }[][] = [];
    this.bien2.forEach(exp=>{

      exs.push([
        {
          columns:[

            [


             {

                text:exp.cantidad,fontSize:5
              },
              {

                text: exp.nombre,fontSize:5
              },

            ]

          ],

        }

      ]);

    });




   
     const desc: { columns: ( { text: string,fontSize:4 })[][]; }[][] = [];
     const cant: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const cuent: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const subcuent: { columns: ({ text:string, fontSize:4 })[][]; }[][] = [];
     const analit1: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const analit2: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const rotu: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const vunitario: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const vtotal: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const fecha: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const estad: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const name_cuenta: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const name_subcuenta: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const name_analitico1: { columns: ({ text: string,fontSize:4})[][]; }[][] = [];
     const name_analitico2: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];

     const fecha_ingresada: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];

     const origen: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     const signo: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
     this.nuevoArray.forEach(exp=>{


      name_cuenta.push([
         {
           columns:[

             [



               {

                 text: exp.nombre_cuenta,fontSize:4
               }
             ]

           ],

         }
       ],

       );

      name_subcuenta.push([
         {
           columns:[

             [



               {

                 text: exp.nombre_subcuenta,fontSize:4
               }
             ]

           ],

         }
       ],

       );

       name_analitico1.push([
         {
           columns:[

             [



               {

                 text: exp.nombre_analitico1,fontSize:4
               }
             ]

           ],

         }
       ],

       );

       name_analitico2.push([
         {
           columns:[

             [



               {

                 text: exp.nombre_analitico2,fontSize:4
               }
             ]

           ],

         }
       ],

       );

       desc.push([
         {
           columns:[

             [




               {

                 text: (exp.nombre_cuenta!=null ? exp.nombre_cuenta : " "),fontSize:4

               },
               {

                 text: (exp.nombre_subcuenta!=null ? exp.nombre_subcuenta : " "),fontSize:4

               },
               {

                 text: (exp.nombre_analitico1!=null || exp.nombre_analitico1==='""' ? exp.nombre_analitico1 : " "),fontSize:4

               },
               {

                 text: ( exp.nombre_analitico2 !=='' ? exp.nombre_analitico2 : "---------------------------------------------"),fontSize:4

               },
               {

                 text: (exp.descripcion!=null  || exp.descripcion==='""' ? exp.descripcion:" "),fontSize:4

               },

               {

                 text: '____________________________________________________',fontSize:4

               },

             ]

           ],

         }

       ]);

       cant.push([
         {
           columns:[

             [


              {

                 text:exp.cantidad,fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },

             ]

           ],

         }

       ]);
       cuent.push([
         {
           columns:[

             [


              {

                 text:(exp.cuenta !=='' ? exp.cuenta : "00"),fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },


               {

                 text: '_______',fontSize:4

               },


             ],


           ],

         }

       ]);

       subcuent.push([
         {
           columns:[

             [



               {

                 text:'\n',fontSize:4
               },
              {

                 text:(exp.subcuenta !=='' ? exp.subcuenta : "00"),fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },

               {

                 text: '_______',fontSize:4

               },
             ]

           ],

         }

       ]);

       analit1.push([
         {
           columns:[

             [


               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },

              {

                 text:(exp.analitico1 !==''? exp.analitico1 : "00"),fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },

               {

                 text: '_______',fontSize:4

               },

             ]

           ],

         }

       ]);

       analit2.push([
         {
           columns:[

             [

               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:(exp.analitico2 !==''? exp.analitico2: "00"),fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text: '_______',fontSize:4

               },



             ]

           ],

         }

       ]);

      rotu.push([
         {
           columns:[

             [


              {

                 text:exp.rotulado,fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },


             ]

           ],

         }

       ]);

      vunitario.push([
         {
           columns:[

             [


              {

                 text:exp.valorUnitario.toLocaleString("de-DE"),fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },


             ]

           ],

         }

       ]);

      vtotal.push([
         {
           columns:[

             [


              {

                 text:exp.valorTotal.toLocaleString("de-DE"),fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },


             ]

           ],

         }

       ]);
      fecha.push([
         {
           columns:[

             [


              {

                 text:exp.fecha,fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },


             ]

           ],

         }

       ]);
      estad.push([
         {
           columns:[

             [


              {

                 text:exp.estado,fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },
               {

                 text:'\n',fontSize:4
               },


             ]

           ],

         }

       ]);
      fecha_ingresada.push([
        {
          columns:[

            [


             {

                text:exp.fecha_ingresada,fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },


            ]

          ],

        }

      ]);
      origen.push([
        {
          columns:[

            [


             {

                text:(exp.estado_conservacion != "null" ? exp.estado_conservacion: " "),fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },


            ]

          ],

        }

      ]);
      signo.push([
        {
          columns:[

            [


             {
              text:("signo"),fontSize:4

                //text:("exp.signo" != "null" ? "exp.signo": " "),fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },
              {

                text:'\n',fontSize:4
              },


            ]

          ],

        }

      ]);



     });


     const pdfInformeDefinition:any = {
     content: [
       [{
         image: this.ImagePath,
         width: 70,
         height: 60,
       },[{text: '', style: 'header',alignment: 'left'}]],
       , [{text: 'INVENTARIO DE BIENES DE USO', style: 'subheader',alignment: 'center'}],
      , [,{text: '', style: 'header',alignment: 'right',fontSize:8},{text: 'FC-03                                                                                                                                                                          Nro. : '+allData.numero_informe, style: 'header',alignment: 'left',fontSize:7}],
       {
         style: 'tableExample',
         table: {

          body: [

            [
                {
                      ul: [
                        {
                          style: 'tableExample',
                          table: {
                            headerRows: 1,
                            body: [
                              [{text: 'ENTIDAD', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                            ]
                          },listType: 'none'
                        },
                        {
                          style: 'tableExample',
                          table: {
                            headerRows: 1,
                            body: [
                              [{text: 'UNIDAD JERARQUICA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                            ]
                          },listType: 'none'
                        },
                        {
                          style: 'tableExample',
                          table: {
                            headerRows: 1,
                            body: [
                              [{text: 'REPARTICION', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                            ]
                          },listType: 'none'
                        },
                        {
                          style: 'tableExample',
                          table: {
                            headerRows: 1,
                            body: [
                              [{text: 'DEPENDENCIA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                            ]
                          },listType: 'none'
                        },
                        {
                          style: 'tableExample',
                          table: {
                            headerRows: 1,
                            body: [
                              [{text: 'AREA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                            ]
                          },listType: 'none'
                        }



                      ],colSpan: 3},
                      {},{},
                  { ul: [
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: '23-18', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                        ]
                      },listType: 'none'
                    },
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: '', fontSize: 4,alignment: 'left',border: [false, false, false, false],margin:[0,0,0,4]}]
                        ]
                      },listType: 'none'
                    },
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: allData.reparticion, fontSize: 4,alignment: 'left',border: [false, false, false, false],margin:[0,0,0,1]}]
                        ]
                      },listType: 'none'
                    },
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text:allData.dependencia, fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                        ]
                      },listType: 'none'
                    },
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: '', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                        ]
                      },listType: 'none'
                    },
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: '', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                        ]
                      },listType: 'none'
                    },
                    ],
                  },{ ul: [
                {text: 'SERV. NAC. DE CALIDAD Y SANIDAD VEGETAL Y DE SEMILLAS', fontSize:4 ,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},

                {text:"", fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 6]},

                {text:allData.reparticion, fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},

                {text:allData.dependencia, fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},

                {text:allData.area, fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},


                   ]}


                  ,{ ul: [

                    {text: 'ESTADO DE CONSERVACION', fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                    String(estad) ==="MB."?{text:'MB..x..MUY BUENO' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]}:{text:'MB.....MUY BUENO' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                    String(estad)==="B."?{text: 'B..X..BUENO' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]}:{text: 'B.....BUENO' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                    String(estad)==="R."?{text:'R..X..REGULAR' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]}:{text:'R.....REGULAR' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                    String(estad)==="M."?{text:'M..X..MALO' , fontSize:5,listType: 'none',margin: [0, 0, 0, 2]}:{text:'M.....MALO' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},

                  ],colSpan: 3},
                  {},{}
                , {

                  ul: [
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: 'BIENES', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                        ]
                      },listType: 'none'
                    },
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: 'NR..........No Registrado', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                        ]
                      },listType: 'none'
                    },
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: 'F..........Faltante', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                        ]
                      },listType: 'none'
                    },
                    {
                      style: 'tableExample',
                      table: {
                        headerRows: 1,

                        body: [
                          [{text: 'C...........Conforme', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                        ]
                      },listType: 'none'
                    },

                  ],colSpan: 5},
                  {},{},{},{}

                ,{ ul: [
                  {
                    style: 'tableExample',
                    table: {
                      headerRows: 1,

                      body: [
                        [{text: 'Fecha: '+allData.fecha_informe, fontSize: 4,alignment: 'left',border: [false, false, false, false],margin: [0, 0, 0, 2]}]
                      ]
                    },listType: 'none'
                  },
                  {
                    style: 'tableExample',
                    table: {
                      headerRows: 1,

                      body: [
                        [{text:'Lugar: Asunción', fontSize: 4,alignment: 'left',border: [false, false, false, false],margin: [0, 0, 0, 5]}]
                      ]
                    },listType: 'none'
                  },

                ]}
            ],

            [
              { text: 'CUENTA', fontSize: 4, alignment: 'center'},
              { text: 'SUB CUENTA', fontSize: 4, alignment: 'center'},
              { text: 'ANALITICO 1', fontSize: 4, alignment: 'center'},
              { text: 'ANALITICO 2', fontSize: 4, alignment: 'center'},
              { text: 'DESCRIPCION', fontSize: 4, alignment: 'center'},


              { text: 'FECHA DE ADQUISIC. Y/O INCORP.', fontSize: 4, alignment: 'center'},
              { text: 'ROTULADO', fontSize: 4, alignment: 'center'},
              { text: 'CANTIDAD', fontSize: 4, alignment: 'center'},
              { text: 'VALOR UNITARIO', fontSize: 4, alignment: 'center'},
              { text: 'VALOR TOTAL', fontSize: 4, alignment: 'center'},


              { text: 'BIENES', fontSize: 4, alignment: 'center'},
              { text: 'ESTADO DE CONSERVACION', fontSize: 4, alignment: 'center'},
              { text: 'DIFERENCIA LIBRO CON INVETARIO FISICO', fontSize: 4, alignment: 'center'},
              { text: 'OBSERVACION', fontSize: 4, alignment: 'center'},
            ],
            [
              cuent,
              subcuent,
              analit1,
              analit2,
              desc,
              fecha,
              rotu,
              cant,
              vunitario,
              vtotal,




              { text: ' ', fontSize: 4, alignment: 'center'},
              estad,
              { text: '  ', fontSize: 4, alignment: 'center'},
              { text: ' ', fontSize: 4, alignment: 'center'}
            ],


            [{ text: 'TOTALES', fontSize: 5, alignment: 'right',colSpan: 9},
            {},{},{},{},{},{},{},{},
            //{ text: Math.round(this.totalGeneral).toLocaleString("de-DE"), fontSize: 5, alignment: 'left',colSpan: 5},
            { text: Math.round(allData.valor_total).toLocaleString("de-DE"), fontSize: 5, alignment: 'left',colSpan: 5},
           {},{},{},{}],


          ]
        }

       },
       {
         style: 'tableExample',
         table: {
           body: [
             [



             ],
           ]
         },
         layout: {
           defaultBorder: false,
         }
       },
       {
         style: 'tableExample',
         table: {
           body: [
             [




               [{ text: '  Jefe de Patrimonio  ', fontSize: 8,decoration: 'overline',margin: [5, 50, 0, 0] }],


             ],
           ]
         },
         layout: {
           defaultBorder: false
         }
       },

       {
         style: 'tableExample',
         table: {
           body: [

             [


              [{ text: '  Director de Finanzas  ', fontSize: 8,decoration: 'overline',margin: [130, -15, 50, 0] }],


             ],
           ]
         },
         layout: {
           defaultBorder: false,
         }
       },

       {
        style: 'tableExample',
        table: {
          body: [

            [


             [{ text: ' Dir. Gral. de Administracion y Finanzas ', fontSize: 8,decoration: 'overline',margin: [230, -21, 0, 0] }],


            ],
          ]
        },
        layout: {
          defaultBorder: false,
        }
      },
       {
         style: 'tableExample',
         table: {
           body: [
             [



              [{ text: '  Presidente Senave  ', fontSize: 8,decoration: 'overline',margin: [400, -26, 0, 0] }],


             ],
           ]
         },
         layout: {
           defaultBorder: false
         }
       },

       {
         style: 'tableExample',
         table: {
           body: [
             [




              [{ text: '   ', fontSize: 8,margin: [195, -50, 0, 0] }],

             ],
             [




               [{ text: '   ', fontSize: 8,margin: [200, -30, 0, 0] }],

              ],

           ]
         },
         layout: {
           defaultBorder: false,
         }
       },

     ]
    }

     const pdf = pdfMake.createPdf(pdfInformeDefinition);
     pdf.open();
}


generarInformefc04(data:string,allData:fc04model){
  this.itemsRecibido2=[]
  this.nuevoArray2=[]
  this.itemsRecibido2=JSON.parse(data);
  this.nuevoArray2 = this.itemsRecibido2;


  const exs: { columns: ({ text: number,fontSize:5 } | { text: string,fontSize:5})[][]; }[][] = [];
  this.bien2.forEach(exp=>{

    exs.push([
      {
        columns:[

          [


           {

              text:exp.cantidad,fontSize:5
            },
            {

              text: exp.nombre,fontSize:5
            },

          ]

        ],

      }

    ]);

  });


   const desc: { columns: ( { text: string,fontSize:4 })[][]; }[][] = [];
   const cant: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const cuent: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const subcuent: { columns: ({ text:string, fontSize:4 })[][]; }[][] = [];
   const analit1: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const analit2: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const rotu: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const vunitario: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const vtotal: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const fecha: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const estad: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const name_cuenta: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const name_subcuenta: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const name_analitico1: { columns: ({ text: string,fontSize:4})[][]; }[][] = [];
   const name_analitico2: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const numero: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const espacio: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const vida_util: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const factura: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const fecha_ingresada: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const tipo: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const origen: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   const signo: { columns: ({ text: string,fontSize:4 })[][]; }[][] = [];
   this.nuevoArray2.forEach(exp=>{


     name_cuenta.push([
       {
         columns:[

           [



             {

               text: exp.nombre_cuenta,fontSize:4
             }
           ]

         ],

       }
     ],

     );

     name_subcuenta.push([
       {
         columns:[

           [



             {

               text: exp.nombre_subcuenta,fontSize:4
             }
           ]

         ],

       }
     ],

     );

     name_analitico1.push([
       {
         columns:[

           [



             {

               text: exp.nombre_analitico1,fontSize:4
             }
           ]

         ],

       }
     ],

     );

     name_analitico2.push([
       {
         columns:[

           [



             {

               text: exp.nombre_analitico2,fontSize:4
             }
           ]

         ],

       }
     ],

     );

     desc.push([
       {
         columns:[

           [




             {

               text: (exp.nombre_cuenta!=null ? exp.nombre_cuenta : " "),fontSize:4

             },
             {

               text: (exp.nombre_subcuenta!=null ? exp.nombre_subcuenta : " "),fontSize:4

             },
             {

               text: (exp.nombre_analitico1!=null || exp.nombre_analitico1==='""' ? exp.nombre_analitico1 : " "),fontSize:4

             },
             {

               text: ( exp.nombre_analitico2 !=='' ? exp.nombre_analitico2 : "---------------------------------------------"),fontSize:4

             },
             {

               text: (exp.descripcion!=null  || exp.descripcion==='""' ? exp.descripcion:" "),fontSize:4

             },
             {

               text: '____________________________________________________',fontSize:4

             },

           ]

         ],

       }

     ]);

     cant.push([
       {
         columns:[

           [


            {

               text:exp.cantidad,fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },

           ]

         ],

       }

     ]);
     cuent.push([
       {
         columns:[

           [


            {

               text:(exp.cuenta !=='' ? exp.cuenta : "00"),fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },


             {

               text: '_______',fontSize:4

             },


           ],


         ],

       }

     ]);

     subcuent.push([
       {
         columns:[

           [



             {

               text:'\n',fontSize:4
             },
            {

               text:(exp.subcuenta !=='' ? exp.subcuenta : "00"),fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },

             {

               text: '_______',fontSize:4

             },
           ]

         ],

       }

     ]);

     analit1.push([
       {
         columns:[

           [


             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },

            {

               text:(exp.analitico1 !==''? exp.analitico1 : "00"),fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },

             {

               text: '_______',fontSize:4

             },

           ]

         ],

       }

     ]);

     analit2.push([
       {
         columns:[

           [

             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:(exp.analitico2 !==''? exp.analitico2: "00"),fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text: '_______',fontSize:4

             },



           ]

         ],

       }

     ]);

     rotu.push([
       {
         columns:[

           [


            {

               text:exp.rotulo,fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },


           ]

         ],

       }

     ]);

     vunitario.push([
       {
         columns:[

           [


            {

               text:exp.valorUnitario.toLocaleString("de-DE"),fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },


           ]

         ],

       }

     ]);

     vtotal.push([
       {
         columns:[

           [


            {

               text:exp.valorTotal.toLocaleString("de-DE"),fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },


           ]

         ],

       }

     ]);

     fecha.push([
       {
         columns:[

           [


            {

               text:(exp.fecha !== "--null" ? exp.fecha : " "),fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },


           ]

         ],

       }

     ]);

     estad.push([
       {
         columns:[

           [


            {

               text:(exp.estado != "null" ? exp.estado : " "),fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },
             {

               text:'\n',fontSize:4
             },


           ]

         ],

       }

     ]);

     vida_util.push([
      {
        columns:[

          [


           {

              text:(exp.vida_util != "null" ? exp.vida_util: " "),fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },


          ]

        ],

      }

    ]);

    fecha_ingresada.push([
      {
        columns:[

          [


           {

              text:(exp.fecha_ingresada != "--null" ? exp.fecha_ingresada: " "),fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },


          ]

        ],

      }

    ]);

    factura.push([
      {
        columns:[

          [


           {

              text:(exp.factura != "null" ? exp.factura: " "),fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },


          ]

        ],

      }

    ]);
    tipo.push([
      {
        columns:[

          [


           {

              text:(exp.tipo != "null" ? exp.tipo: " "),fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },


          ]

        ],

      }

    ]);
    origen.push([
      {
        columns:[

          [


           {

              text:(exp.origen != "null" ? exp.origen: " "),fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },


          ]

        ],

      }

    ]);
    signo.push([
      {
        columns:[

          [


           {

              text:(exp.signo != "null" ? exp.signo: " "),fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },
            {

              text:'\n',fontSize:4
            },


          ]

        ],

      }

    ]);



   });


   const pdfInformeDefinition:any = {
   content: [
     [{
       image: this.ImagePath,
       width: 70,
       height: 60,
     },[{text: '', style: 'header',alignment: 'left'}]],
     , [{text: ' MOVIMIENTO DE BIENES DE USO', style: 'subheader',alignment: 'center'}],
    , [,{text: '', style: 'header',alignment: 'right',fontSize:8},{text: 'FC-04                                         DEPENDENCIA REMITENTE                                                       DEPENDENCIA RECEPTORA                         Nro. : '+allData.nro_informe, style: 'header',alignment: 'left',fontSize:7}],
     {
       style: 'tableExample',
       table: {

        body: [

          [
              {
                    ul: [
                      {
                        style: 'tableExample',
                        table: {
                          headerRows: 1,
                          body: [
                            [{text: 'ENTIDAD', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                          ]
                        },listType: 'none'
                      },
                      {
                        style: 'tableExample',
                        table: {
                          headerRows: 1,
                          body: [
                            [{text: 'UNIDAD JERARQUICA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                          ]
                        },listType: 'none'
                      },
                      {
                        style: 'tableExample',
                        table: {
                          headerRows: 1,
                          body: [
                            [{text: 'REPARTICION', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                          ]
                        },listType: 'none'
                      },
                      {
                        style: 'tableExample',
                        table: {
                          headerRows: 1,
                          body: [
                            [{text: 'DEPENDENCIA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                          ]
                        },listType: 'none'
                      },
                      {
                        style: 'tableExample',
                        table: {
                          headerRows: 1,
                          body: [
                            [{text: 'AREA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                          ]
                        },listType: 'none'
                      },
                      {
                        style: 'tableExample',
                        table: {
                          headerRows: 1,
                          body: [
                            [{text: 'LUGAR', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                          ]
                        },listType: 'none'
                      }


                    ],colSpan: 3},
                    {},{},
            { ul: [
              {
                style: 'tableExample',
                table: {
                  headerRows: 1,

                  body: [
                    [{text: '23-18', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                  ]
                },listType: 'none'
              },
              {
                style: 'tableExample',
                table: {
                  headerRows: 1,

                  body: [
                    [{text: '', fontSize: 4,alignment: 'left',border: [false, false, false, false],margin:[0,0,0,4]}]
                  ]
                },listType: 'none'
              },
              {
                style: 'tableExample',
                table: {
                  headerRows: 1,

                  body: [
                    [{text: allData.reparticion, fontSize: 4,alignment: 'left',border: [false, false, false, false],margin:[0,0,0,1]}]
                  ]
                },listType: 'none'
              },
              {
                style: 'tableExample',
                table: {
                  headerRows: 1,

                  body: [
                    [{text: allData.dependencia, fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                  ]
                },listType: 'none'
              },
              {
                style: 'tableExample',
                table: {
                  headerRows: 1,

                  body: [
                    [{text: '', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                  ]
                },listType: 'none'
              },
              {
                style: 'tableExample',
                table: {
                  headerRows: 1,

                  body: [
                    [{text: '', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                  ]
                },listType: 'none'
              },
              ],
            },{ ul: [
              {text: 'SERV. NAC. DE CALIDAD Y SANIDAD VEGETAL Y DE SEMILLAS', fontSize:4 ,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},

              {text: allData.unidad_jerarquica, fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 6]},

              {text:allData.reparticion_text, fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},

              {text:allData.dependencia, fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},

              {text:"", fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},

              {text:"", fontSize: 4,alignment: 'left',listType: 'none',margin: [0, 0, 0, 2]},

              ]}


                ,{ ul: [

                  {text: 'ORIGEN O MOVIMIENTO', fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                  allData.origen_movimiento==="ALTA"?{text:'A..x..ALTA' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]}:{text:'A.....ALTA' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                  allData.origen_movimiento==="BAJA"?{text: 'B..X..BAJA' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]}:{text: 'B.....BAJA' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                  allData.origen_movimiento==="COMPRA"?{text:'C..X..COMPRA' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]}:{text:'C.....COMPRA' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                  allData.origen_movimiento==="DONACIONES"?{text:'D..X..DONACIONES' , fontSize:5,listType: 'none',margin: [0, 0, 0, 2]}:{text:'D.....DONACIONES' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                  allData.origen_movimiento==="TRANSPASO"?{text:'A..X..TRANSPASO' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]}:{text:  'A.....TRANSPASO' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                  allData.origen_movimiento==="REPARACION"?{text:'RM..X..REPARACION MAYOR' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]}:{text:  'RM.....REPARACION MAYOR' , fontSize: 5,listType: 'none',margin: [0, 0, 0, 2]},
                ],colSpan: 3},
                {},{}
              , {

                ul: [
                  {
                    style: 'tableExample',
                    table: {
                      headerRows: 1,

                      body: [
                        [{text: 'ENTIDAD', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                      ]
                    },listType: 'none'
                  },
                  {
                    style: 'tableExample',
                    table: {
                      headerRows: 1,

                      body: [
                        [{text: 'UNIDAD JERAQUICA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                      ]
                    },listType: 'none'
                  },
                  {
                    style: 'tableExample',
                    table: {
                      headerRows: 1,

                      body: [
                        [{text: 'REPARTICION', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                      ]
                    },listType: 'none'
                  },
                  {
                    style: 'tableExample',
                    table: {
                      headerRows: 1,

                      body: [
                        [{text: 'DEPENDENCIA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                      ]
                    },listType: 'none'
                  },
                  {
                    style: 'tableExample',
                    table: {
                      headerRows: 1,

                      body: [
                        [{text: 'AREA', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                      ]
                    },listType: 'none'
                  },
                  {
                    style: 'tableExample',
                    table: {
                      headerRows: 1,

                      body: [
                        [{text: 'LUGAR', fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                      ]
                    },listType: 'none'
                  },
                ],colSpan: 3},
                {},{}

              ,{ ul: [
                {
                  style: 'tableExample',
                  table: {
                    headerRows: 1,

                    body: [
                      [{text: allData.entidad_text, fontSize: 4,alignment: 'left',border: [false, false, false, false],margin: [0, 0, 0, 2]}]
                    ]
                  },listType: 'none'
                },
                {
                  style: 'tableExample',
                  table: {
                    headerRows: 1,

                    body: [
                      [{text: allData.entidad , fontSize: 4,alignment: 'left',border: [false, false, false, false],margin: [0, 0, 0, 5]}]
                    ]
                  },listType: 'none'
                },
                {
                  style: 'tableExample',
                  table: {
                    headerRows: 1,

                    body: [
                      [{text: allData.reparticion+'-'+allData.reparticion_text, fontSize: 4,alignment: 'left',border: [false, false, false, false],margin: [0, 0, 0, 1]}]
                    ]
                  },listType: 'none'
                },
                {
                  style: 'tableExample',
                  table: {
                    headerRows: 1,

                    body: [
                      [{text: allData.dependencia+'-'+allData.dependencia, fontSize: 4,alignment: 'left',border: [false, false, false, false],margin: [0, 0, 0, 2]}]
                    ]
                  },listType: 'none'
                },
                {
                  style: 'tableExample',
                  table: {
                    headerRows: 1,

                    body: [
                      [{text: "", fontSize: 4,alignment: 'left',border: [false, false, false, false],margin: [0, 0, 0, 2]}]
                    ]
                  },listType: 'none'
                },
                {
                  style: 'tableExample',
                  table: {
                    headerRows: 1,

                    body: [
                      [{text: "", fontSize: 4,alignment: 'left',border: [false, false, false, false]}]
                    ]
                  },listType: 'none'
                },
              ],colSpan: 5},
              {},{},{},{}
          ],

          [
            { text: 'CUENTA', fontSize: 4, alignment: 'center'},
            { text: 'SUB CUENTA', fontSize: 4, alignment: 'center'},
            { text: 'ANALITICO 1', fontSize: 4, alignment: 'center'},
            { text: 'ANALITICO 2', fontSize: 4, alignment: 'center'},
            { text: 'DESCRIPCION', fontSize: 4, alignment: 'center'},


            { text: 'FECHA', fontSize: 4, alignment: 'center'},
            { text: 'TIPO', fontSize: 4, alignment: 'center'},
            { text: 'Nº', fontSize: 4, alignment: 'center'},
            { text: 'ROTULADO', fontSize: 4, alignment: 'center'},
            { text: 'CANTIDAD', fontSize: 4, alignment: 'center'},
            { text: 'VALOR UNITARIO', fontSize: 4, alignment: 'center'},
            { text: 'VALOR TOTAL', fontSize: 4, alignment: 'center'},
            { text: 'SIGNO', fontSize: 4, alignment: 'center'},
            { text: 'FECHA DE INCORP. O MOVIM.', fontSize: 4, alignment: 'center'},
            { text: 'AÑOS DE VIDA UTIL', fontSize: 4, alignment: 'center'},
            { text: 'ORIGEN O MOVIM.', fontSize: 4, alignment: 'center'}
          ],
          [
            cuent,
            subcuent,
            analit1,
            analit2,
            desc,
            fecha,
            tipo,
            factura,
            rotu,
            cant,
            vunitario,
            vtotal,
            signo,
            fecha_ingresada,
            vida_util,
            origen
          ],

          /*[{ text: ' ', fontSize: 5, style: 'tableHeader', colSpan: 6, alignment: 'center'},{},{},{},{},{},{ text: 'SUB TOTAL', fontSize: 5, alignment: 'center',style: 'tableHeader', colSpan: 2},{},
          { text: Math.round(this.totalGeneral).toLocaleString("de-DE"), fontSize: 5, alignment: 'center'},{},{},{},{},{},{},{}],
          [{ text: ' ', fontSize: 5, style: 'tableHeader', colSpan: 6, alignment: 'center'},{},{},{},{},{},{ text: 'I.V.A', fontSize: 5, alignment: 'center',style: 'tableHeader', colSpan: 2},{},
          { text: Math.round(this.totalGeneral*this.IVA).toLocaleString("de-DE"), fontSize: 5, alignment: 'center'},{},{},{},{},{},{},{}],*/

          [{ text: 'SUB TOTAL', fontSize: 5, alignment: 'right',colSpan: 11},
          {},{},{},{},{},{},{},{},{},{},
          { text: allData.sub_total, fontSize: 5, alignment: 'left',colSpan: 5},
         {},{},{},{}],
         [{ text: 'I.V.A', fontSize: 5, alignment: 'right',colSpan: 11},
         {},{},{},{},{},{},{},{},{},{},
         { text: allData.iva, fontSize: 5, alignment: 'left',colSpan: 5},
        {},{},{},{}],
          [{text: 'Obs: '+allData.observacion, fontSize: 5, alignment: 'lengh',colSpan: 9},{},{},{},{},{},{},{},{},{ text: 'TOTALES', fontSize: 5, alignment: 'right',colSpan: 2},
          {},
          { text: allData.totales, fontSize: 5, alignment: 'left',colSpan: 5},
         {},{},{},{}]

        ]
      }

     },
     {
       style: 'tableExample',
       table: {
         body: [
           [
            [{ text: 'Recibí de conformidad los bienes detallados en el presente formulario' , fontSize: 6,margin: [0, 0, 0, 0] }],


           ],
         ]
       },
       layout: {
         defaultBorder: false,
       }
     },
     {
       style: 'tableExample',
       table: {
         body: [
           [


            /*[{ text: 'OBSERVACION:' , fontSize: 6,margin: [0, 0, 0, 0] },{ text: 'DEVUELTO EN:', fontSize: 8,margin: [390, 35, -30, 0] }],
            */

             [{ text: '  Jefe de Patrimonio  ', fontSize: 8,decoration: 'overline',margin: [5, 45, 0, 0] }],


           ],
         ]
       },
       layout: {
         defaultBorder: false
       }
     },

     {
       style: 'tableExample',
       table: {
         body: [

           [



            /*[{text:'Lugar:',fontSize: 6,margin: [390, -5, 0, 0] },
            {text:'Fecha:',fontSize: 6,margin: [390,4 , 0, 0] },{text:'Responsable de recepción:',fontSize: 6,margin: [390, 4, 0, 0] }],
            */

           ],
         ]
       },
       layout: {
         defaultBorder: false,
       }
     },
     {
       style: 'tableExample',
       table: {
         body: [
           [



            [{ text: '  Jefe de Dependencia Receptora  ', fontSize: 8,decoration: 'overline',margin: [390, -20, 0, 0] }],

            /* [{ text: 'ENTREGADO EN:', fontSize: 8,margin: [5, -45, 0, 0],alignment: 'left'},{text:'Lugar:',fontSize: 6,margin: [5, 4, 0, 0] },
             {text:'Fecha: ',fontSize: 6,margin: [5,4 , 0, 0] }],*/


           ],
         ]
       },
       layout: {
         defaultBorder: false
       }
     },

     {
       style: 'tableExample',
       table: {
         body: [
           [




            [{ text: '   ', fontSize: 8,margin: [195, -50, 0, 0] }],

           ],
           [




             [{ text: '   ', fontSize: 8,margin: [200, -30, 0, 0] }],

            ],

         ]
       },
       layout: {
         defaultBorder: false,
       }
     },

   ]
   }


   const pdf = pdfMake.createPdf(pdfInformeDefinition);
   pdf.open();

   /*this.modelofc04={
    cont_informe: 0,
    dependencia: '',
    entidad: '',
    entidad_text: '',
    fecha_informe: '',
    items: '',
    iva: '',
    nro_informe: '',
    observacion: '',
    origen_movimiento: '',
    reparticion: '',
    reparticion_text: '',
    sub_total: '',
    totales: '',
    unidad_jerarquica: '',
    cuenta:''
  }
  console.log("esto es lo que no funciona: ",String(this.nuevoArray));
  for (let i = 0; i < this.nuevoArray.length; i++) {
    this.modelofc04={
      cont_informe:0,
      dependencia:String(this.dependText2),
      entidad:'23-18',
      entidad_text:'SERV. NAC. DE CALIDAD Y SANIDAD VEGETAL Y DE SEMILLAS',
      fecha_informe:'valor',
      items:String(JSON.stringify(this.nuevoArray)),
      iva:String(Math.round(this.totalGeneral*this.IVA).toLocaleString("de-DE")),
      nro_informe:String(this.contador)+"/"+String(this.year),
      observacion:String(this.observaciones),
      origen_movimiento:String(this.origen) ,
      reparticion:String(this.repart),
      reparticion_text:String(this.repartText2),
      sub_total:String(Math.round(this.totalGeneral).toLocaleString("de-DE")),
      totales:String(Math.round(this.totalGeneral+(this.totalGeneral*this.IVA)).toLocaleString("de-DE")),
      unidad_jerarquica:'valor',
      cuenta:String(this.nuevoArray[i].cuenta)
    }

  }
  this.saveDataInformefc04(this.modelofc04);

  this.nuevoArray=[];*/
}


generarInformefc10(data:string,allData:fc10model){
  this.itemsRecibido3=[]
  this.nuevoArray3=[]
  this.itemsRecibido3=JSON.parse(data);
  this.nuevoArray3 = this.itemsRecibido3;

  console.log("allData:"+allData);

  const desc: { columns: ( { text: string,fontSize:5})[][]; }[][] = [];
  const cant: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const cuent: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const subcuent: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const analit1: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const analit2: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const rotu: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const vunitario: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const vtotal: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const fecha: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const estad: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const name_cuenta: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const name_subcuenta: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const name_analitico1: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const name_analitico2: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  const espacio: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
  this.nuevoArray3.forEach(exp=>{


    name_cuenta.push([
      {
        columns:[

          [



            {

              text: exp.nombre_cuenta,fontSize:5
            }
          ]

        ],

      }
    ],

    );

    name_subcuenta.push([
      {
        columns:[

          [



            {

              text: exp.nombre_subcuenta,fontSize:5
            }
          ]

        ],

      }
    ],

    );

    name_analitico1.push([
      {
        columns:[

          [



            {

              text: exp.nombre_analitico1,fontSize:5
            }
          ]

        ],

      }
    ],

    );

    name_analitico2.push([
      {
        columns:[

          [



            {

              text: exp.nombre_analitico2,fontSize:5
            }
          ]

        ],

      }
    ],

    );

    desc.push([
      {
        columns:[

          [




            {

              text: (exp.nombre_cuenta!=null ? exp.nombre_cuenta : " "),fontSize:5

            },
            {

              text: (exp.nombre_subcuenta!=null ? exp.nombre_subcuenta : " "),fontSize:5

            },
            {

              text: (exp.nombre_analitico1!=null || exp.nombre_analitico1==='""' ? exp.nombre_analitico1 : " "),fontSize:5

            },
            {

              text: ( exp.nombre_analitico2 !=='' ? exp.nombre_analitico2 : "---------------------------------------------"),fontSize:5

            },
            {

              text: (exp.descripcion!=null  || exp.descripcion==='""' ? exp.descripcion:" "),fontSize:5

            },
            {

              text: '____________________________________________________',fontSize:5

            },

          ]

        ],

      }

    ]);

    cant.push([
      {
        columns:[

          [


           {

              text:exp.cantidad,fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },

          ]

        ],

      }

    ]);
    cuent.push([
      {
        columns:[

          [


           {

              text:(exp.cuenta !=='' ? exp.cuenta : "00"),fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },


            {

              text: '_______',fontSize:5

            },


          ],


        ],

      }

    ]);

    subcuent.push([
      {
        columns:[

          [



            {

              text:'\n',fontSize:5
            },
           {

              text:(exp.subcuenta !=='' ? exp.subcuenta : "00"),fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },

            {

              text: '_______',fontSize:5

            },
          ]

        ],

      }

    ]);

    analit1.push([
      {
        columns:[

          [


            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },

           {

              text:(exp.analitico1 !==''? exp.analitico1 : "00"),fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },

            {

              text: '_______',fontSize:5

            },

          ]

        ],

      }

    ]);

    analit2.push([
      {
        columns:[

          [

            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:(exp.analitico2 !==''? exp.analitico2: "00"),fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text: '_______',fontSize:5

            },



          ]

        ],

      }

    ]);

    rotu.push([
      {
        columns:[

          [


           {

              text:exp.rotulo,fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },


          ]

        ],

      }

    ]);

    vunitario.push([
      {
        columns:[

          [


           {

              text:exp.valorUnitario.toLocaleString("de-DE"),fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },


          ]

        ],

      }

    ]);

    vtotal.push([
      {
        columns:[

          [


           {

              text:exp.valorTotal.toLocaleString("de-DE"),fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },


          ]

        ],

      }

    ]);

    fecha.push([
      {
        columns:[

          [


           {

              text:(exp.fecha !== "--null" ? exp.fecha : " "),fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },


          ]

        ],

      }

    ]);

    estad.push([
      {
        columns:[

          [


           {

              text:(exp.estado != "null" ? exp.estado : " "),fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },
            {

              text:'\n',fontSize:5
            },


          ]

        ],

      }

    ]);

  });


  const pdfInformeDefinition:any = {
  content: [
    [{
      image: this.ImagePath,
      width: 70,
      height: 60,
    },[{text: '', style: 'header',alignment: 'left'}]],
    , [{text: ' PLANILLA DE RESPONSABILIDAD INDIVIDUAL', style: 'subheader',alignment: 'center'}],
   , [,{text: '', style: 'header',alignment: 'right',fontSize:8},{text: 'FC-10                                                                                                                                                                                                                            Nro. : '+allData.numero_informe, style: 'header',alignment: 'left',fontSize:7}],
    {
      style: 'tableExample',
      table: {

        body: [
          [
            {

            ul: [
              {	text: 'ENTIDAD:', fontSize:6,listType: 'none',margin: [0, 0, 0, 10] },
              {text: 'UNIDAD JERARQUICA:', fontSize: 6,listType: 'none'},
              {text: 'REPARTICION:', fontSize: 6,listType: 'none'},
              {text:'DEPENDENCIA:', fontSize: 6,listType: 'none'},
              {text: 'AREA', fontSize: 6,listType: 'none'},
            ],colSpan: 4

          },{},{},{},{
            ul: [
            {text: '23-18', fontSize: 6,listType: 'none',margin: [0, 0, 0, 10]},
            {text: ' ', fontSize: 6,listType: 'none'},
            {text: allData.reparticion, fontSize: 5,listType: 'none'},
            {text: allData.dependencia, fontSize: 5,listType: 'none'},
            {text: ' ', fontSize: 6,listType: 'none'}
          ]},
          {ul: [
            {text: 'SERV. NAC. DE CALIDAD Y SANIDAD VEGETAL Y DE SEMILLAS', fontSize: 6,listType: 'none',margin: [0, 0, 0, 2]},
            {text: ' ', fontSize: 6,listType: 'none'},
            {text: allData.reparticion_text, fontSize: 5,listType: 'none'},
            {text:allData.dependencia_text, fontSize: 5,listType: 'none'},
            {text: ' ', fontSize: 6,listType: 'none'}
          ]},
          { ul: [
            {text: 'Nombre del Funcionario Responsable: '+allData.responsable, fontSize: 6,listType: 'none'},
            {text: ' ', fontSize: 6,listType: 'none'},
            {text: '', fontSize: 6,listType: 'none'},
            {text:'Cargo que desempeña: '+allData.cargo, fontSize: 6,listType: 'none'},
            {text: ' ', fontSize: 6,listType: 'none'}
          ],colSpan:5
          },{},{},{},{}],
          [ { text: 'CUENTA', fontSize: 5, alignment: 'center'}, { text: 'SUB CUENTA', fontSize: 5, alignment: 'center'}, { text: 'ANALITICO 1', fontSize: 5, alignment: 'center'}
          , { text: 'ANALITICO 2', fontSize: 5, alignment: 'center'}, { text: 'ROTULADO', fontSize: 5, alignment: 'center'}, { text: 'DESCRIPCION', fontSize: 5, alignment: 'center'},
          { text: 'Cantidad', fontSize: 5, alignment: 'center'}, { text: 'Valor Unitario', fontSize: 5, alignment: 'center'}, { text: 'Valor Total', fontSize: 5, alignment: 'center'},
          { text: 'Fecha de Adqui. o Incorp.', fontSize: 5, alignment: 'center'}, { text: 'Estado de conservac.', fontSize: 5, alignment: 'center'}],
          [cuent,subcuent,analit1,analit2,rotu,desc,cant,vunitario,vtotal,fecha,estad],
         /* [{ text: ' ', fontSize: 5, style: 'tableHeader', colSpan: 6, alignment: 'center'},{},{},{},{},{},{ text: 'SUB TOTAL', fontSize: 5, alignment: 'center',style: 'tableHeader', colSpan: 2},{},
          { text: Math.round(this.totalGeneral).toLocaleString("de-DE"), fontSize: 5, alignment: 'center'},{},{}],
          [{ text: ' ', fontSize: 5, style: 'tableHeader', colSpan: 6, alignment: 'center'},{},{},{},{},{},{ text: 'I.V.A', fontSize: 5, alignment: 'center',style: 'tableHeader', colSpan: 2},{},
          { text: Math.round(this.totalGeneral*this.IVA).toLocaleString("de-DE"), fontSize: 5, alignment: 'center'},{},{}],*/
          [{ text: ' ', fontSize: 5, style: 'tableHeader', colSpan: 6, alignment: 'center'},{},{},{},{},{},{ text: 'TOTAL GENERAL', fontSize: 5, alignment: 'center',style: 'tableHeader', colSpan: 2},{},
          { text: allData.total, fontSize: 5, alignment: 'center'},{},{}]
        ]
      }

    },
    {
      style: 'tableExample',
      table: {
        body: [
          [
           [{ text: 'Recibí de conformidad los bienes detallados en el presente formulario' , fontSize: 6,margin: [0, 0, 0, 0] }],


          ],
        ]
      },
      layout: {
        defaultBorder: false,
      }
    },
    {
      style: 'tableExample',
      table: {
        body: [
          [


           [{ text: 'OBSERVACION:' , fontSize: 6,margin: [0, 0, 0, 0] },{ text: 'DEVUELTO EN:', fontSize: 8,margin: [390, 35, -30, 0] }],

          ],
        ]
      },
      layout: {
        defaultBorder: false
      }
    },

    {
      style: 'tableExample',
      table: {
        body: [

          [



           [{text:'Lugar:',fontSize: 6,margin: [390, -5, 0, 0] },
           {text:'Fecha:',fontSize: 6,margin: [390,4 , 0, 0] },{text:'Responsable de recepción:',fontSize: 6,margin: [390, 4, 0, 0] }],


          ],
        ]
      },
      layout: {
        defaultBorder: false,
      }
    },
    {
      style: 'tableExample',
      table: {
        body: [
          [





            [{ text: 'ENTREGADO EN:', fontSize: 8,margin: [5, -45, 0, 0],alignment: 'left'},{text:'Lugar:',fontSize: 6,margin: [5, 4, 0, 0] },
            {text:'Fecha: ',fontSize: 6,margin: [5,4 , 0, 0] }],


          ],
        ]
      },
      layout: {
        defaultBorder: false
      }
    },

    {
      style: 'tableExample',
      table: {
        body: [
          [




           [{ text: '  Firma Funcionario Responsable  ', fontSize: 8,decoration: 'overline',margin: [195, -60, 0, 0] }],

          ],
          [




            [{ text: '  Vº Bº Jefe de Dependencia  ', fontSize: 8,decoration: 'overline',margin: [200, -30, 0, 0] }],

           ],

        ]
      },
      layout: {
        defaultBorder: false,
      }
    },

  ]
  }


  const pdf = pdfMake.createPdf(pdfInformeDefinition);
  pdf.open();


}



}
