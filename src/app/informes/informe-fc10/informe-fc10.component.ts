import { Component, OnInit } from '@angular/core';
import * as pdfMake from 'pdfmake/build/pdfmake';
import { Funcionarios } from '../informe-ts/funcionarios';
import { InformesService } from '../informes.service';
import { pluck} from 'rxjs/operators';
import { Empleado } from '../informe-ts/empleado';
import { Bienes } from '../informe-ts/bienes';
import { ItemDetails2 } from '../informe-ts/ItemDetails2';
import { Prod2 } from '../informe-ts/descripcion2';
import { Prod3 } from '../informe-ts/descripcion3';
import { Router } from '@angular/router';
import { ItemDetailsFilter } from '../informe-ts/itemDetailsFilter';
import { Location } from '@angular/common';
import swal from 'sweetalert2';
import { Prod } from '../informe-ts/descripcion';
import { InformesApiService } from '../informes-api.service';
import { Info } from '../informe-ts/api_info_contador';
import { fc10model } from '../informe-ts/fc10/modelo_fc10';
import { Funcionarios2 } from '../informe-ts/funcionarios2';
import { itemProveedor } from '../informe-ts/fc10/data_item_proveedor';
import { nameProvider } from '../informe-ts/fc10/data_name_proveedor';



@Component({
  selector: 'app-informe-fc10',
  templateUrl: './informe-fc10.component.html'
})
export class InformeFc10Component implements OnInit {
  public datofc10!:fc10model[];
  public modelofc10:fc10model={
    dependencia: '',
    dependencia_text: '',
    entidad: '',
    entidad_text: '',
    fecha_informe: '',
    numero_informe: '',
    reparticion: '',
    reparticion_text: '',
    unidad_jerarquica: '',
    responsable: '',
    cargo: '',
    items: '',
    cont_informe: 0,
    cuenta: '',
    total: ''
  }

  newInformeContador:Info = {
    informe:""
  }
  searchText:string="";
  searchText2:string="";
  searchTextProveedor:string="";
  searchTextFuncionario:string="";

  public contendorProveedor:itemProveedor[]=[];
  public nombresProveedores:nameProvider[]=[];

  public infoGenerado:boolean=true;
  public codigobien:string="";
  public results:ItemDetailsFilter[]=[]
  resultadoConsulta!: ItemDetailsFilter;
  public tipoInfo=true;
  public valorInforme="total";
  public ImagePath: string= "";
  public funcionarios : Funcionarios2[] = [];
  public datos :ItemDetails2[] = [];
  public datos1 :ItemDetailsFilter[] = [];
  public datosRecibidos :ItemDetails2[] = [];

  public datoItem:ItemDetailsFilter[]=[];
  public habilitarBoton:boolean=false;
  public emp:Empleado[]=[];
  public bienes : Bienes[] = [];
  public bien:Prod2[]=[];
  public repart!:string;
  public depend!:string;
  public repartText!:string;
  public dependText!:string;
  public localizacion!:string;
  public cantidadBienesLocalizado:number=0;

  public totalGeneral:number=0;
  public IVA:number=0.1;

  public formato_fecha:string = "";
  public formato_fecha2:string = "";
  nuevoBien!:Prod2;
  public contador:number=0;

  public hoy = new Date();
  public year = this.hoy.getFullYear();

  nuevoResponsable: Prod3={
    nombre:"",
    cargo:""

  }
  nuevoArray: Prod2[]=[];

  nuevo: Prod2={
    cuenta: "",
    nombre_cuenta: "",
    subcuenta: "",
    nombre_subcuenta: "",
    analitico1: "",
    nombre_analitico1: "",
    analitico2: "",
    nombre_analitico2: "",
    rotulo: "",
    descripcion: "",
    cantidad: "",
    valorUnitario: 0,
    valorTotal: 0,
    fecha: "",
    estado: "",
    vida_util: "",
    fecha_ingresada: "",
    factura: "",
    signo: "",
    origen: "",
    tipo: ""
  }

  nuevo2:Prod={
    nombre:"",
    cantidad:0,
    rotulo:"",
    vida_util:"",
    fecha:"",
    factura:"",
    tipo:""
  };

  arraydesp={
    reparticion:1,
    despachos:[
      {
        num:1,

      }
    ]
  }

  dependencia:string[]=[];

  dependencia1:string[]=['DESPACHO','SECRETARIA GENERAL','AUDITORIA INTERNA INSTITUCIONAL','ASESORIA JURIDICA','DIRECCION DE GESTION DE PERSONAS'];
  dependencia2:string[]=['DESPACHO']
  datoContador: string | null | undefined;

  constructor(private servicio:InformesService,private informeServices:InformesService,
    public router:Router, public _location: Location,private apiService:InformesApiService) {

    }


  ngOnInit(): void {
    this.infoGenerado=true;


    this.habilitarBoton=false;
    this.getContadorInformeS("fc10");
    this.mostrarFuncionarios();
    this.mostrarDatos();

    this.getProveedoresAll();

    this.mostrarBienes();
    this.ImagePath ='data:image/jpeg;base64,';''
  }



  tipoInforme(tipo:string):void{

    if(tipo==="total"){
      this.nuevoInforme();
      this.totalGeneral=0;
      this.nuevoArray=[];

      this.tipoInfo = true;
      this.valorInforme="total";

    }else{

      this.totalGeneral=0;
      this.nuevoArray=[];

      this.tipoInfo = false;
      this.valorInforme="parcial";
      this.mostrarDatos2();

    }


  }

nuevoInforme(){
  this.router.navigateByUrl("/refresh", { skipLocationChange: true }).then(() => {
		console.log(decodeURI(this._location.path()));
		this.router.navigate([decodeURI(this._location.path())]);
		});

}

 seleccion(dat1:string) {

   console.log(dat1);

   const result =  this.datos.filter((obj) => {
    return obj.parent_description === dat1+' ';
    });
    this.datosRecibidos=result;
    console.log("es lo que hay",result)
 }

 seleccionarLocalizacion(){
   console.log("localizacion:",this.localizacion);
   this.cantidadBienesLocalizado=0;
   this.servicio.filtrarBien2(this.localizacion).pipe(
    pluck('data')
  )
  .subscribe(data => {
    this.totalGeneral=0;
    this.nuevo={
      cuenta:"",
      nombre_cuenta:"",
      subcuenta:"",
      nombre_subcuenta:"",
      analitico1:"",
      nombre_analitico1:"",
      analitico2:"",
      nombre_analitico2:"",
      rotulo:"",
      descripcion:"",
      cantidad:"",
      valorUnitario:0,
      valorTotal:0,
      fecha:"",
      estado:"",
      origen:"",
      signo:"",
      factura:"",
      tipo:"",
      vida_util:"",
      fecha_ingresada:""
    }
    this.nuevoArray=[];

    console.log(data.length);
    this.cantidadBienesLocalizado= data.length;
    this.datoItem=data;
    //console.log(this.datoItem);

    //filtro por fecha
    if (this.searchText2.length > 0){
      //console.log("filtro por fecha:",this.datoItem.filter(arr => arr.purchase_date == this.searchText2).length);
      this.datoItem = this.datoItem.filter(arr => arr.purchase_date == this.searchText2);
      this.cantidadBienesLocalizado = this.datoItem.length;
    }
    //filtro por proveedor
    if(this.contendorProveedor.length>0){
      console.log("Esto es contendorProveedor",this.contendorProveedor);
      console.log("Esto es datoItem",this.datoItem);
      this.datoItem = this.datoItem.filter(arr => this.contendorProveedor.find(s => s.id_item == arr.id));
      console.log("filtrando por proveedor id_company",this.datoItem.length);
      console.log("filtrando por proveedor id_company",this.datoItem);

      this.cantidadBienesLocalizado = this.datoItem.length;
    }


    for (let i = 0; i < this.datoItem.length; i++) {

      this.nuevo={
        cuenta:"",
        nombre_cuenta:"",
        subcuenta:"",
        nombre_subcuenta:"",
        analitico1:"",
        nombre_analitico1:"",
        analitico2:"",
        nombre_analitico2:"",
        rotulo:"",
        descripcion:"",
        cantidad:"",
        valorUnitario:0,
        valorTotal:0,
        fecha:"",
        estado:"",
        origen:"",
        signo:"",
        factura:"",
        tipo:"",
        vida_util:"",
        fecha_ingresada:""
      }
      this.formato_fecha="";
      this.formato_fecha2="";

      console.log("iteracion",i);
      if(this.datoItem[i].custom_fields_values[0].value !==null){
        this.nuevo.cuenta = JSON.stringify( this.datoItem[i].custom_fields_values[0].value).substring(1,6).replace(/['"]+/g, '');
        this.nuevo.nombre_cuenta = JSON.stringify(this.datoItem[i].custom_fields_values[0].value).substring(6,this.datoItem[i].custom_fields_values[0].value.length+1);
      }
      this.nuevo.subcuenta = JSON.stringify(this.datoItem[i].custom_fields_values[1].value).substring(6,8);
      this.nuevo.nombre_subcuenta = JSON.stringify(this.datoItem[i].custom_fields_values[1].value).substring(8,this.datoItem[i].custom_fields_values[1].value.length+1).replace(/['"]+/g, '');
      this.nuevo.analitico1 =JSON.stringify(this.datoItem[i].custom_fields_values[2].value).substring(8,11);
      this.nuevo.nombre_analitico1 = JSON.stringify(this.datoItem[i].custom_fields_values[2].value).substring(11,this.datoItem[i].custom_fields_values[2].value.length+1).replace(/['"]+/g, '');
      if(this.datoItem[i].custom_fields_values[3].value !==null){
        this.nuevo.analitico2 =JSON.stringify(this.datoItem[i].custom_fields_values[3].value).substring(10,12);
        this.nuevo.nombre_analitico2 = JSON.stringify(this.datoItem[i].custom_fields_values[3].value).substring(12,this.datoItem[i].custom_fields_values[3].value.length+1).replace(/['"]+/g, '');
      }


      this.nuevo.rotulo= JSON.stringify(this.datoItem[i].barcode).replace(/['"]+/g, '');
      this.nuevo.descripcion= JSON.stringify(this.datoItem[i].description).replace(/['"]+/g, '');
      this.nuevo.cantidad="1";
      this.nuevo.valorUnitario= Number(this.datoItem[i].total_cost);
      this.nuevo.valorTotal= Number(this.datoItem[i].total_cost);

      if (this.datoItem[i].purchase_date != null){
        this.formato_fecha = JSON.stringify(this.datoItem[i].purchase_date).replace(/['"]+/g, '').replace(/['-]+/g, '');;
        console.log("fecha",this.formato_fecha);
        this.formato_fecha2 += this.formato_fecha.substring(6,9)+"-"+this.formato_fecha.substring(4,6)+"-"+this.formato_fecha.substring(0,4);
        this.nuevo.fecha=this.formato_fecha2;
        console.log("fecha formateada",this.nuevo.fecha);
      }
      if (this.datoItem[i].custom_fields_values[4].value != null){
        this.nuevo.estado= JSON.stringify(this.datoItem[i].custom_fields_values[4].value).replace(/['"]+/g, '');
      }
      this.totalGeneral +=this.nuevo.valorUnitario;

      this.nuevoArray.push(this.nuevo);

    }


    console.log("este es",this.nuevoArray);

  },err => {
    console.log('HTTP Error', err)
  }
  );
 }
 seleccionar(nuevo:ItemDetailsFilter){
 /*  console.log(nuevo.custom_fields_values[3]);
   this.nuevo.cuenta = JSON.stringify(nuevo.custom_fields_values[0].value).substring(1,6).replace(/['"]+/g, '');
   this.nuevo.subcuenta = JSON.stringify(nuevo.custom_fields_values[1].value).substring(6,8);
   this.nuevo.analitico1 =JSON.stringify(nuevo.custom_fields_values[2].value).substring(8,11);
   this.nuevo.analitico2 =JSON.stringify(nuevo.custom_fields_values[3].value).substring(10,12);
   this.nuevo.rotulo= JSON.stringify(nuevo.barcode).replace(/['"]+/g, '');
   this.nuevo.descripcion= JSON.stringify(nuevo.description).replace(/['"]+/g, '');
   this.nuevo.cantidad="1";
   this.nuevo.valorUnitario= Number(nuevo.total_cost);
   this.nuevo.valorTotal= Number(nuevo.total_cost);
   this.nuevo.fecha= JSON.stringify(nuevo.purchase_date);
   this.nuevo.estado= "MB";
   console.log("cuenta:",this.nuevo.cuenta);
   console.log("subcuenta:",this.nuevo.subcuenta);
   console.log("analitico1:",this.nuevo.analitico1);
   console.log("analitico2:",this.nuevo.analitico2);
   console.log("rotulo:",this.nuevo.rotulo);
   console.log("descricpion:",this.nuevo.descripcion);
   console.log("cantidad:",this.nuevo.cantidad);
   console.log("valor unitario:",this.nuevo.valorUnitario);
   console.log("valor total:",this.nuevo.valorTotal);
   console.log("fecha:",this.nuevo.fecha);
   console.log("estado",this.nuevo.estado);
/*

 }
  agregar(){

    this.bien.push(this.nuevo);
    if(this.nuevo.valorUnitario != null){
      this.totalGeneral = this.totalGeneral + this.nuevo.valorUnitario;
    }

    this.nuevo={
      cuenta:"",
      subcuenta:"",
      analitico1:"",
      analitico2:"",
      rotulo:"",
      descripcion:"",
      cantidad:"",
      valorUnitario:0,
      valorTotal:0,
      fecha:"",
      estado:""
    }
   /* if(this.nuevoBien.nombre.trim().length === 0){ return;}
    this.nuevoBien.cantidad=1;
    this.bien.push(this.nuevoBien);
    console.log("nuevo",this.nuevoBien);
    this.nuevoBien = {
      nombre: '',
      cantidad: 0,
      cuenta:0,
      subcuenta:0,
      analitico1:0,
      analitico2:0

    }*/
  }

  separarReparticion(){
    this.repartText= this.repart.substring(this.repart.indexOf("-")+1,this.repart.length);
    this.repart=this.repart.substring(0,this.repart.indexOf("-"));
    console.log("Reparticion texto:",this.repartText);
    console.log("Reparticion codigo:",this.repart);
  }
  separarDependencia(){

    this.dependText= this.depend.substring(this.depend.indexOf("-")+1,this.depend.length);
    this.depend=this.depend.substring(0,this.depend.indexOf("-"));
    console.log("Dependencia texto:",this.dependText);
    console.log("Dependencia codigo:",this.depend);

  }

  cambios(){
    if( this.nuevoResponsable.nombre.indexOf("-") === -1){

    }else{
      this.nuevoResponsable.cargo = this.nuevoResponsable.nombre.substring(this.nuevoResponsable.nombre.indexOf("-")+1,this.nuevoResponsable.nombre.length);
      this.nuevoResponsable.nombre= this.nuevoResponsable.nombre.substring(0,this.nuevoResponsable.nombre.indexOf("-"));

    }
  }



  mostrarBienes():void{
    this.informeServices.todosBienes().pipe(
      pluck('data')
    )
    .subscribe(data => {
      this.bienes=data;
      console.log(data);
    });
  }

  mostrarFuncionarios():void{
    this.servicio.todosFuncionarios2().pipe(
      //pluck('data')
      //pluck('rh_funnombre')

    )
    .subscribe(data => {
      this.funcionarios=data;
      console.log("funcionarios",this.funcionarios);
    });
  }

  eliminar( dato:number):void{

        this.totalGeneral -=this.nuevoArray[dato].valorUnitario;

        this.nuevoArray.splice(dato, 1);

  }


  guardar():void{

    //this.totalGeneral=0;
    this.nuevo={
      cuenta:"",
      nombre_cuenta:"",
      subcuenta:"",
      nombre_subcuenta:"",
      analitico1:"",
      nombre_analitico1:"",
      analitico2:"",
      nombre_analitico2:"",
      rotulo:this.nuevo2.rotulo,
      descripcion:"",
      cantidad:"1",
      valorUnitario:0,
      valorTotal:0,
      fecha:"",
      estado:"",
      vida_util:this.nuevo2.vida_util,
      fecha_ingresada:this.nuevo2.fecha,
      factura:this.nuevo2.factura,
      tipo: 'factura',
      origen: 'C',
      signo: ''
    }


    console.log('esto es results:',this.results);

    console.log('esto es nuevo2:',this.nuevo2);


      for(let i = 0; i < this.results.length; i++) {

       /* this.nuevo={
          cuenta:"",
          nombre_cuenta:"",
          subcuenta:"",
          nombre_subcuenta:"",
          analitico1:"",
          nombre_analitico1:"",
          analitico2:"",
          nombre_analitico2:"",
          rotulo:this.nuevo2.rotulo,
          descripcion:"",
          cantidad:"1",
          valorUnitario:0,
          valorTotal:0,
          fecha:"",
          estado:"",
          vida_util:this.nuevo2.vida_util,
          fecha_ingresada:this.nuevo2.fecha,
          factura:this.nuevo2.factura
        }*/
          this.formato_fecha="";
          this.formato_fecha2="";

          if(this.results[i] !== null){
            this.nuevo.cuenta = JSON.stringify( this.results[i].custom_fields_values[0].value).substring(1,6).replace(/['"]+/g, '');
            this.nuevo.nombre_cuenta = JSON.stringify(this.results[i].custom_fields_values[0].value).substring(6,this.results[i].custom_fields_values[0].value.length+1);
          }
          if(this.results[i].custom_fields_values[1].value !== null){
            this.nuevo.subcuenta = JSON.stringify(this.results[i].custom_fields_values[1].value).substring(6,8);
            this.nuevo.nombre_subcuenta = JSON.stringify(this.results[i].custom_fields_values[1].value).substring(8,this.results[i].custom_fields_values[1].value.length+1).replace(/['"]+/g, '');
          }
            if(this.results[i].custom_fields_values[2].value !== null){
              this.nuevo.analitico1 =JSON.stringify(this.results[i].custom_fields_values[2].value).substring(8,11);
              this.nuevo.nombre_analitico1 = JSON.stringify(this.results[i].custom_fields_values[2].value).substring(11,this.results[i].custom_fields_values[2].value.length+1).replace(/['"]+/g, '');
            }
            if(this.results[i].custom_fields_values[3].value !== null){
              this.nuevo.analitico2 =JSON.stringify(this.results[i].custom_fields_values[3].value).substring(10,12);
              this.nuevo.nombre_analitico2 = JSON.stringify(this.results[i].custom_fields_values[3].value).substring(12,this.results[i].custom_fields_values[3].value.length+1).replace(/['"]+/g, '');
            }



            this.nuevo.rotulo=this.codigobien;

            if(this.results[i].anual_depreciation !== null){
              if(JSON.stringify(this.results[i].anual_depreciation) === '25'){
                this.nuevo.vida_util="4";
              }
              if(JSON.stringify(this.results[i].anual_depreciation) === '10'){
                this.nuevo.vida_util="10";
              }

            }



         // this.nuevo.rotulo= JSON.stringify(this.results[i].barcode).replace(/['"]+/g, '');
          this.nuevo.descripcion= JSON.stringify(this.results[i].description).replace(/['"]+/g, '');
          this.nuevo.cantidad="1";
          this.nuevo.valorUnitario= Number(this.results[i].total_cost);
          this.nuevo.valorTotal= Number(this.results[i].total_cost);


          this.formato_fecha = JSON.stringify(this.results[i].purchase_date).replace(/['"]+/g, '').replace(/['-]+/g, '');;
          console.log("fecha",this.formato_fecha);
          this.formato_fecha2 += this.formato_fecha.substring(6,9)+"-"+this.formato_fecha.substring(4,6)+"-"+this.formato_fecha.substring(0,4);
          this.nuevo.fecha=this.formato_fecha2;
          console.log("fecha formateada",this.nuevo.fecha);

          this.nuevo.fecha_ingresada=this.formato_fecha2

          this.nuevo.estado= JSON.stringify(this.results[i].custom_fields_values[4].value).replace(/['"]+/g, '');

          this.totalGeneral +=this.nuevo.valorUnitario;

        this.nuevoArray.push(this.nuevo);

        this.nuevo2.cantidad=1;


        this.nuevo2={
          nombre:"",
          cantidad:0,
          rotulo:"",
          vida_util:"",
          fecha:"",
          factura:"",
          tipo:""
        };

      }




    console.log("este es",this.nuevoArray);


  }


  buscar(){

    let key: keyof ItemDetails2;

    for (key in this.resultadoConsulta){
      delete this.resultadoConsulta[key];
    }


    this.results=this.search(this.codigobien);
    this.resultadoConsulta = this.results[0];
    console.log("resultado de la locura",this.resultadoConsulta);
  }

  search( name:string) {
    let entry;

      return this.datos1.filter(
        function(data){ return data.barcode == name }
    );
  }

  mostrarDatos():void{

    this.servicio.todosDatos2().pipe(
      pluck('data')

    )
    .subscribe( resp =>
      this.datos=resp

    );
  }

  mostrarDatos2():void{

    this.datos1=[];
    swal({
      title: 'Cargando datos...',
      allowOutsideClick: false
    });

    swal.showLoading();



    this.informeServices.todosDatos3().pipe(
      pluck('data')

    )
    .subscribe( resp =>{
      this.datos1=resp;
      console.log('bienes:',resp);
      swal.close();
    }

    );


  }

  /*saveContadorInformef(info:Info):void{
    console.log('entra aqui2');
    this.apiService.saveContadorInforme(info).pipe(
      pluck('status')
    )
    .subscribe(data => {
      console.log("nuevo informe:",data);
    });

    this.infoGenerado=false;
  }*/

  getContadorInformeS(informe:string):void{
    this.contador= 0;

    this.apiService.getContadorInforme("fc10").pipe(
      pluck('data')

    )
    .subscribe(data => {

     this.contador=Number(data)+1;

    });

  }

  getProveedoresAll():void{
    this.servicio.todosProveedores().pipe(
      pluck('data')

    )
    .subscribe(data => {

      this.nombresProveedores=data;
      console.log("todos los proveedores:",this.nombresProveedores);

    });
  }

  getProveedoresFilter(nombre:string):void{
    this.servicio.filtrarProveedores(nombre).pipe(
      pluck('data')

    )
    .subscribe(data => {

      this.contendorProveedor=data;
      console.log("proveedor buscado:",this.contendorProveedor);

    });
  }


  nuevoInformeR(){
    this.router.navigateByUrl("/refresh", { skipLocationChange: true }).then(() => {
      console.log(decodeURI(this._location.path()));
      this.router.navigate([decodeURI(this._location.path())]);
      });

  }

  seleccionarProveedores(){
    if(this.nombresProveedores.length !== 0){
      this.getProveedoresFilter(this.searchTextProveedor);
    }

  }

  generarInforme2(){


    /**********************************************************/

      //this.getProveedoresFilter(this.searchTextProveedor);
    /**********************************************************/



    this.cantidadBienesLocalizado = 0;
    this.cantidadBienesLocalizado = this.nuevoArray.length;
    this.habilitarBoton=true;
    const desc: { columns: ( { text: string,fontSize:5})[][]; }[][] = [];
    const cant: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const cuent: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const subcuent: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const analit1: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const analit2: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const rotu: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const vunitario: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const vtotal: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const fecha: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const estad: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const name_cuenta: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const name_subcuenta: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const name_analitico1: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const name_analitico2: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    const espacio: { columns: ({ text: string,fontSize:5 })[][]; }[][] = [];
    this.nuevoArray.forEach(exp=>{


      name_cuenta.push([
        {
          columns:[

            [



              {

                text: exp.nombre_cuenta,fontSize:5
              }
            ]

          ],

        }
      ],

      );

      name_subcuenta.push([
        {
          columns:[

            [



              {

                text: exp.nombre_subcuenta,fontSize:5
              }
            ]

          ],

        }
      ],

      );

      name_analitico1.push([
        {
          columns:[

            [



              {

                text: exp.nombre_analitico1,fontSize:5
              }
            ]

          ],

        }
      ],

      );

      name_analitico2.push([
        {
          columns:[

            [



              {

                text: exp.nombre_analitico2,fontSize:5
              }
            ]

          ],

        }
      ],

      );

      desc.push([
        {
          columns:[

            [




              {

                text: (exp.nombre_cuenta!=null ? exp.nombre_cuenta : " "),fontSize:5

              },
              {

                text: (exp.nombre_subcuenta!=null ? exp.nombre_subcuenta : " "),fontSize:5

              },
              {

                text: (exp.nombre_analitico1!=null || exp.nombre_analitico1==='""' ? exp.nombre_analitico1 : " "),fontSize:5

              },
              {

                text: ( exp.nombre_analitico2 !=='' ? exp.nombre_analitico2 : "---------------------------------------------"),fontSize:5

              },
              {

                text: (exp.descripcion!=null  || exp.descripcion==='""' ? exp.descripcion:" "),fontSize:5

              },
              {

                text: '____________________________________________________',fontSize:5

              },

            ]

          ],

        }

      ]);

      cant.push([
        {
          columns:[

            [


             {

                text:exp.cantidad,fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },

            ]

          ],

        }

      ]);
      cuent.push([
        {
          columns:[

            [


             {

                text:(exp.cuenta !=='' ? exp.cuenta : "00"),fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },


              {

                text: '_______',fontSize:5

              },


            ],


          ],

        }

      ]);

      subcuent.push([
        {
          columns:[

            [



              {

                text:'\n',fontSize:5
              },
             {

                text:(exp.subcuenta !=='' ? exp.subcuenta : "00"),fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },

              {

                text: '_______',fontSize:5

              },
            ]

          ],

        }

      ]);

      analit1.push([
        {
          columns:[

            [


              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },

             {

                text:(exp.analitico1 !==''? exp.analitico1 : "00"),fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },

              {

                text: '_______',fontSize:5

              },

            ]

          ],

        }

      ]);

      analit2.push([
        {
          columns:[

            [

              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:(exp.analitico2 !==''? exp.analitico2: "00"),fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text: '_______',fontSize:5

              },



            ]

          ],

        }

      ]);

      rotu.push([
        {
          columns:[

            [


             {

                text:exp.rotulo,fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },


            ]

          ],

        }

      ]);

      vunitario.push([
        {
          columns:[

            [


             {

                text:exp.valorUnitario.toLocaleString("de-DE"),fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },


            ]

          ],

        }

      ]);

      vtotal.push([
        {
          columns:[

            [


             {

                text:exp.valorTotal.toLocaleString("de-DE"),fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },


            ]

          ],

        }

      ]);

      fecha.push([
        {
          columns:[

            [


             {

                text:(exp.fecha !== "--null" ? exp.fecha : " "),fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },


            ]

          ],

        }

      ]);

      estad.push([
        {
          columns:[

            [


             {

                text:(exp.estado != "null" ? exp.estado : " "),fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },
              {

                text:'\n',fontSize:5
              },


            ]

          ],

        }

      ]);

    });


    const pdfInformeDefinition:any = {
    content: [
      [{
        image: this.ImagePath,
        width: 70,
        height: 60,
      },[{text: '', style: 'header',alignment: 'left'}]],
      , [{text: ' PLANILLA DE RESPONSABILIDAD INDIVIDUAL', style: 'subheader',alignment: 'center'}],
     , [,{text: '', style: 'header',alignment: 'right',fontSize:8},{text: 'FC-10                                                                                                                                                                                                                            Nro. : '+this.contador+"/"+this.year, style: 'header',alignment: 'left',fontSize:7}],
      {
        style: 'tableExample',
        table: {

          body: [
            [
              {

              ul: [
                {	text: 'ENTIDAD:', fontSize:6,listType: 'none',margin: [0, 0, 0, 10] },
                {text: 'UNIDAD JERARQUICA:', fontSize: 6,listType: 'none'},
                {text: 'REPARTICION:', fontSize: 6,listType: 'none'},
                {text:'DEPENDENCIA:', fontSize: 6,listType: 'none'},
                {text: 'AREA', fontSize: 6,listType: 'none'},
              ],colSpan: 4

            },{},{},{},{
              ul: [
              {text: '23-18', fontSize: 6,listType: 'none',margin: [0, 0, 0, 10]},
              {text: ' ', fontSize: 6,listType: 'none'},
              {text: this.repart, fontSize: 5,listType: 'none'},
              {text: this.depend, fontSize: 5,listType: 'none'},
              {text: ' ', fontSize: 6,listType: 'none'}
            ]},
            {ul: [
              {text: 'SERV. NAC. DE CALIDAD Y SANIDAD VEGETAL Y DE SEMILLAS', fontSize: 6,listType: 'none',margin: [0, 0, 0, 2]},
              {text: ' ', fontSize: 6,listType: 'none'},
              {text: this.repartText, fontSize: 5,listType: 'none'},
              {text:this.dependText, fontSize: 5,listType: 'none'},
              {text: ' ', fontSize: 6,listType: 'none'}
            ]},
            { ul: [
              {text: 'Nombre del Funcionario Responsable: '+this.nuevoResponsable.nombre, fontSize: 6,listType: 'none'},
              {text: ' ', fontSize: 6,listType: 'none'},
              {text: '', fontSize: 6,listType: 'none'},
              {text:'Cargo que desempeña: '+this.nuevoResponsable.cargo, fontSize: 6,listType: 'none'},
              {text: ' ', fontSize: 6,listType: 'none'}
            ],colSpan:5
            },{},{},{},{}],
            [ { text: 'CUENTA', fontSize: 5, alignment: 'center'}, { text: 'SUB CUENTA', fontSize: 5, alignment: 'center'}, { text: 'ANALITICO 1', fontSize: 5, alignment: 'center'}
            , { text: 'ANALITICO 2', fontSize: 5, alignment: 'center'}, { text: 'ROTULADO', fontSize: 5, alignment: 'center'}, { text: 'DESCRIPCION', fontSize: 5, alignment: 'center'},
            { text: 'Cantidad', fontSize: 5, alignment: 'center'}, { text: 'Valor Unitario', fontSize: 5, alignment: 'center'}, { text: 'Valor Total', fontSize: 5, alignment: 'center'},
            { text: 'Fecha de Adqui. o Incorp.', fontSize: 5, alignment: 'center'}, { text: 'Estado de conservac.', fontSize: 5, alignment: 'center'}],
            [cuent,subcuent,analit1,analit2,rotu,desc,cant,vunitario,vtotal,fecha,estad],

            [{ text: 'Esta localizacion tiene asignada '+this.cantidadBienesLocalizado+' bienes activos', fontSize: 5, style: 'tableHeader', colSpan: 6, alignment: 'center'},{},{},{},{},{},{ text: 'TOTAL GENERAL', fontSize: 5, alignment: 'center',style: 'tableHeader', colSpan: 2},{},
            { text: Math.round(this.totalGeneral).toLocaleString("de-DE"), fontSize: 5, alignment: 'center'},{},{}]
          ]
        }

      },
      {
        style: 'tableExample',
        table: {
          body: [
            [
             [{ text: 'Recibí de conformidad los bienes detallados en el presente formulario' , fontSize: 6,margin: [0, 0, 0, 0] }],


            ],
          ]
        },
        layout: {
          defaultBorder: false,
        }
      },
      {
        style: 'tableExample',
        table: {
          body: [
            [


             [{ text: 'OBSERVACION:' , fontSize: 6,margin: [0, 0, 0, 0] },{ text: 'DEVUELTO EN:', fontSize: 8,margin: [390, 35, -30, 0] }],

            ],
          ]
        },
        layout: {
          defaultBorder: false
        }
      },

      {
        style: 'tableExample',
        table: {
          body: [

            [



             [{text:'Lugar:',fontSize: 6,margin: [390, -5, 0, 0] },
             {text:'Fecha:',fontSize: 6,margin: [390,4 , 0, 0] },{text:'Responsable de recepción:',fontSize: 6,margin: [390, 4, 0, 0] }],


            ],
          ]
        },
        layout: {
          defaultBorder: false,
        }
      },
      {
        style: 'tableExample',
        table: {
          body: [
            [





              [{ text: 'ENTREGADO EN:', fontSize: 8,margin: [5, -45, 0, 0],alignment: 'left'},{text:'Lugar:',fontSize: 6,margin: [5, 4, 0, 0] },
              {text:'Fecha: ',fontSize: 6,margin: [5,4 , 0, 0] }],


            ],
          ]
        },
        layout: {
          defaultBorder: false
        }
      },

      {
        style: 'tableExample',
        table: {
          body: [
            [




             [{ text: '  Firma Funcionario Responsable  ', fontSize: 8,decoration: 'overline',margin: [195, -60, 0, 0] }],

            ],
            [




              [{ text: '  Vº Bº Jefe de Dependencia  ', fontSize: 8,decoration: 'overline',margin: [200, -30, 0, 0] }],

             ],

          ]
        },
        layout: {
          defaultBorder: false,
        }
      },

    ]
    }


    const pdf = pdfMake.createPdf(pdfInformeDefinition);
    pdf.open();

    this.modelofc10={
      dependencia: '',
      dependencia_text: '',
      entidad: '',
      entidad_text: '',
      fecha_informe: '',
      numero_informe: '',
      reparticion: '',
      reparticion_text: '',
      unidad_jerarquica: '',
      responsable: '',
      cargo: '',
      items: '',
      cont_informe:0,
      cuenta:'',
      total: ''
    }
    console.log("esto es lo que no funciona: ",String(this.nuevoArray));
    for (let i = 0; i < this.nuevoArray.length; i++) {
      this.modelofc10={
            dependencia: String(this.depend),
            dependencia_text: String(this.dependText),
            entidad: '23-18',
            entidad_text: 'SERV. NAC. DE CALIDAD Y SANIDAD VEGETAL Y DE SEMILLAS',
            fecha_informe: '',
            numero_informe: String(this.contador)+"/"+String(this.year),
            reparticion: String(this.repart),
            reparticion_text: String(this.repartText),
            unidad_jerarquica: '',
            responsable: this.nuevoResponsable.nombre,
            cargo: this.nuevoResponsable.cargo,
            items: String(JSON.stringify(this.nuevoArray)),
            cont_informe:0,
            cuenta:String(this.nuevoArray[i].cuenta),
            total:String(Math.round(this.totalGeneral).toLocaleString("de-DE"))
          }
    }


    this.saveDataInformefc10(this.modelofc10);

    this.nuevoArray=[];
    this.nuevoResponsable={
      nombre:"",
      cargo:""
    }
    this.totalGeneral=0

    this.nuevoInforme();
  }

  generarInforme3(){
    const pdfInformeDefinition3={
    content:{
			style: 'tableExample',
			color: '#444',
        table: {

          body: [
            [{text: 'Header with Colspan = 2', style: 'tableHeader', colSpan: 2, alignment: 'center'}, {}, {text: 'Header 3', style: 'tableHeader', alignment: 'center'}],
            [{text: 'Header 1', style: 'tableHeader', alignment: 'center'}, {text: 'Header 2', style: 'tableHeader', alignment: 'center'}, {text: 'Header 3', style: 'tableHeader', alignment: 'center'}],
            ['Sample value 1', 'Sample value 2', 'Sample value 3'],
            [{rowSpan: 3, text: 'rowSpan set to 3\nLorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor'}, 'Sample value 2', 'Sample value 3'],
            ['', 'Sample value 2', 'Sample value 3'],
            ['Sample value 1', 'Sample value 2', 'Sample value 3'],
            ['Sample value 1', {colSpan: 2, rowSpan: 2, text: 'Both:\nrowSpan and colSpan\ncan be defined at the same time'}, ''],
            ['Sample value 1', '', ''],
          ]
        }
		},

    }

    const pdf3 = pdfMake.createPdf(pdfInformeDefinition3);
    pdf3.open();

  }

  saveDataInformefc10(data:fc10model):void{
    console.log('entra aqui');
    this.apiService.saveInformefc10(data).pipe(
      pluck('status')
    )
    .subscribe(data => {
      console.log("nuevo informe fc10:",data);
    });

    this.infoGenerado=false;
  }
}
