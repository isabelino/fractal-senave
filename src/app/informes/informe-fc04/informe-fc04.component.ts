 ...
 public contador:number=0;
  ...
  guardarTotal():void{
   ...
   for (let i = 0; i < this.nuevoArray.length; i++) {
      this.nuevoArray[i].factura = this.nuevo2.factura;
      this.nuevoArray[i].tipo=this.nuevo2.tipo;
   }
   //console.log("se agrega factura y tipo",this.nuevoArray);
   this.facturaTipo="Numero de comprobante y tipo de documento agregado";
  ...
  validarOrigen():string{
    ...
    if (this.origen =="DONACIONES") {
      return "D";
    }
    if (this.origen =="TRANSPASO") {
      return "T";
    }
    if (this.origen =="REPARACION") {
      return "R";
    }
    return "";
  }
...
getYearInforme(informe:string):any{
  this.apiService.getContadorYearInforme("fc04").pipe(
    pluck('data')
  ).subscribe(data =>{
     this.guardarYear( data )

  });
}

guardarYear(valor:string):void{
  this.actual_year=Number(valor);
  console.log('actual_year',this.actual_year)
  this.anio = Number(this.actual_year)
}

getContadorInformeS(informe:string):void{

  this.getYearInforme('fc04')
  this.apiService.getContadorInforme("fc04").pipe(
    pluck('data')

  )
  .subscribe(data2 => {
    if ( Number(this.actual_year) < Number(this.year) ){
      this.contador=1
    }else{
      this.contador = Number(data2)+1
    }

   });

    console.log('anio',this.anio)
    console.log('Esto año actual:',this.year);
    console.log('Esto es el contador:',this.contador);
    console.log('Esto año del ultimo registro:',this.actual_year);
...
nuevoInforme(){
  ...
  generarInforme2(){
    if (this.nuevoArray.length) {

      for (let x = 0; x < this.nuevoArray.length; x++) {
        this.nuevoArray[x].origen= this.validarOrigen();
      ...
      swal({
        title: 'Error!',
        text: 'Debe cargar todos los datos requeridos para generar el informe',
        type: 'error',
  ...
  seleccionarLocalizacion(){
   ...
   .subscribe(data => {
     ...
     for (let i = 0; i < this.datoItem.length; i++) {
       ...
       if(this.datoItem[i].anual_depreciation !== null){
        if(JSON.stringify(this.datoItem[i].anual_depreciation) === '25'){
          this.nuevo.vida_util="4";
        }
        if(JSON.stringify(this.datoItem[i].anual_depreciation) === '10'){
          this.nuevo.vida_util="10";
        }

        if(JSON.stringify(this.datoItem[i].anual_depreciation) === '2.5'){
          this.nuevo.vida_util="40";
        }

        if(JSON.stringify(this.datoItem[i].anual_depreciation) === '20'){
          this.nuevo.vida_util="5";
        }
